...
// ── SOURCES ───────────────────────────────────────────────────────────────────
const SOURCES = [
  { id: "aliexpress", label: "AliExpress", icon: "🛒", color: "#FF6010",
  ...
  { id: "temu", label: "Temu", icon: "🎯", color: "#FF5400",
    // Best sellers sort
...
function PulseBar({ active }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 3, height: 20 }}>
      {[0,1,2,3,4].map(i => (
        <div key={i} style={{ width: 3, borderRadius: 2, background: active ? PULSE_COLORS[i % PULSE_COLORS.length] : "#2a2a3a", animation: active ? `pulse-bar 1.1s ease-in-out ${i*0.15}s infinite` : "none", height: active ? undefined : 8 }} />
      ))}
    </div>
  );
}
...
function TrendSparkline({ data, color }) {
  ...
  const trendColor = lastVal > firstVal ? "#34A853" : lastVal < firstVal ? "#DC2626" : "#FBBF24";
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <div>
        ...
        <div style={{ fontSize: 9, color: "#6b6b8a" }}>{lastVal}/100</div>
      </div>
    </div>
  );
}
...
function TrendCard({ product, index, compact = false, market = "Nigeria" }) {
  ...
  return (
    <div onClick={() => setExpanded(!expanded)} style={{
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
            <span style={{ fontSize: compact ? 11 : 13, fontWeight: 700, color: "#e8e8f5", fontFamily: "'Syne',sans-serif" }}>{product.name}</span>
          ...
          {!compact && product.trendData && (
            <div style={{ marginTop: 8 }}>
              <div style={{ fontSize: 9, color: "#6b6b8a", marginBottom: 4 }}>TREND (12 weeks)</div>
        ...
        <div style={{ textAlign: "center", minWidth: 42 }}>
          ...
          <div style={{ fontSize: 8, color: "#6b6b8a", marginTop: 1 }}>HEAT</div>
          <div style={{ width: 34, height: 3, background: "#2a2a4a", borderRadius: 2, marginTop: 3 }}>
      ...
      {/* Expanded section */}
      {expanded && (
        <div style={{ marginTop: 13, paddingTop: 13, borderTop: "1px solid #2a2a4a" }}>

          {/* AI Insight */}
          {product.insight && (
            <div style={{ fontSize: 12, color: "#a0a0c0", lineHeight: 1.65, marginBottom: 10 }}>
          ...
          {product.trendData && (
            <div style={{ marginBottom: 10, padding: "10px 12px", background: "#0d0d1a", borderRadius: 8 }}>
              <div style={{ fontSize: 10, color: "#6b6b8a", marginBottom: 8 }}>📈 TREND OVER TIME (12 weeks)</div>
          ...
          {product.competitors && product.competitors.length > 0 && (
            <div style={{ marginBottom: 10 }}>
              <div style={{ fontSize: 11, color: "#FBBF24", fontWeight: 600, marginBottom: 6 }}>🏪 Competitors / Dropshippers</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                {product.competitors.map((c, i) => (
                  <div key={i} style={{ padding: "7px 10px", background: "#0d0d1a", borderRadius: 7, border: "1px solid #2a2a4a" }}>
                    <div style={{ fontSize: 11, color: "#e8e8f5", fontWeight: 600 }}>{c.name}</div>
                    <div style={{ fontSize: 10, color: "#6b6b8a", marginTop: 2 }}>{c.platform} · {c.detail}</div>
                    {c.url && (
          ...
          {product.suggestedAction && (
            <div style={{ padding: "8px 11px", background: "#0f4c2a", borderRadius: 8, color: "#4ade80", fontSize: 11, border: "1px solid #16803450", marginBottom: 10 }}>
              <strong>→ Action for Fabian Stores:</strong> {product.suggestedAction}
            </div>
          )}

          {/* All platform links */}
          <div style={{ paddingTop: 10, borderTop: "1px solid #1e1e3a" }}>
            <div style={{ fontSize: 10, color: "#6b6b8a", marginBottom: 6 }}>🔗 Search on all platforms (sorted by sales volume):</div>
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${w} ${h}`} style={{ width: "100%", height: 80 }}>
        ...
        {pts.map((p, i) => (
          <circle key={i} cx={p.x} cy={p.y} r={i === peakIdx ? "2.5" : "1.5"} fill={i === peakIdx ? "#FBBF24" : color} opacity="0.9" />
        ))}
        {/* Peak label */}
        <text x={pts[peakIdx]?.x} y={pts[peakIdx]?.y - 4} textAnchor="middle" fontSize="3.5" fill="#FBBF24">PEAK</text>
      </svg>
      {/* X-axis labels */}
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 2 }}>
        {[0, Math.floor(data.length / 3), Math.floor(data.length * 2 / 3), data.length - 1].map(i => (
          <span key={i} style={{ fontSize: 9, color: "#6b6b8a" }}>{labels[i]}</span>
        ))}
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
        <span style={{ fontSize: 9, color: "#6b6b8a" }}>Low: {min}</span>
        <span style={{ fontSize: 9, color: "#FBBF24" }}>Peak: {max}</span>
...
function CompareColumn({ scan, index, onRemove }) {
  ...
  return (
    <div style={{ flex: "0 0 295px", background: "#12122a", border: `1px solid ${color}40`, borderRadius: 14, overflow: "hidden" }}>
      ...
      {scan.results?.topInsight && (
        <div style={{ padding: "8px 12px", borderBottom: "1px solid #1e1e3a", fontSize: 10, color: "#a0a0c0", lineHeight: 1.5 }}>🎯 {scan.results.topInsight}</div>
      )}
...
function HistoryCard({ scan, onLoad, onDelete }) {
  ...
  return (
    <div style={{ background: "#1a1a2e", border: "1px solid #2a2a4a", borderRadius: 11, padding: "12px 14px", display: "flex", alignItems: "center", gap: 12 }}>
      <div style={{ flex: 1 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: "#e8e8f5", fontFamily: "'Syne',sans-serif" }}>{scan.category}</span>
          <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 20, background: "#FF601015", color: "#FF6010", border: "1px solid #FF601030" }}>{scan.market}</span>
        </div>
        <div style={{ fontSize: 10, color: "#6b6b8a", marginTop: 3 }}>{scan.timestamp} · {scan.results?.products?.length || 0} products · avg heat {heatAvg}</div>
        {topProduct && <div style={{ fontSize: 10, color: "#a0a0c0", marginTop: 3 }}>🥇 {topProduct.name}</div>}
      </div>
      <div style={{ display: "flex", gap: 6 }}>
        <button onClick={() => onLoad(scan)} style={{ padding: "5px 12px", borderRadius: 7, background: "#FF601020", border: "1px solid #FF601040", color: "#FF6010", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>Load</button>
        <button onClick={() => onDelete(scan.id)} style={{ padding: "5px 8px", borderRadius: 7, background: "transparent", border: "1px solid #2a2a4a", color: "#6b6b8a", fontSize: 11, cursor: "pointer" }}>✕</button>
      </div>
    </div>
  );
}
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        select option{background:#1a1a2e;color:#e8e8f5}
      `}</style>

      <div style={{minHeight:"100vh",background:"#0d0d1a",fontFamily:"'DM Sans',sans-serif",color:"#e8e8f5",paddingBottom:40}}>

        {/* ── HEADER ── */}
        <div style={{background:"linear-gradient(180deg,#12122a,#0d0d1a)",borderBottom:"1px solid #1e1e3a",padding:"16px 16px 12px",position:"sticky",top:0,zIndex:100,backdropFilter:"blur(12px)"}}>
          <div style={{maxWidth:980,margin:"0 auto"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:32,height:32,borderRadius:8,background:"linear-gradient(135deg,#FF6010,#FF3C00)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,boxShadow:"0 4px 15px #FF601050",animation:"float 3s ease-in-out infinite"}}>📡</div>
                <div>
                  <div style={{fontSize:18,fontWeight:900,fontFamily:"'Syne',sans-serif",letterSpacing:"-0.5px"}}>Trend<span style={{color:"#FF6010"}}>Pulse</span></div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                ...
                <div style={{display:"flex",gap:2,background:"#1a1a2e",borderRadius:8,padding:3}}>
                  {TABS.map(tab => (
                    <button key={tab.id} onClick={()=>setView(tab.id)} className="tab-btn" style={{padding:"4px 9px",borderRadius:5,border:"none",fontSize:10,fontWeight:600,cursor:"pointer",transition:"all 0.15s",background:view===tab.id?"#FF6010":"transparent",color:view===tab.id?"#fff":"#6b6b8a",fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap"}}>{tab.label}</button>
                  ))}
                </div>
              </div>
            </div>
        ...
        <div style={{maxWidth:980,margin:"0 auto",padding:"16px 14px 0"}}>
          ...
          {view==="results" && <>
            {/* Controls */}
            <div style={{background:"linear-gradient(135deg,#1a1a2e,#12122a)",border:"1px solid #2a2a4a",borderRadius:13,padding:15,marginBottom:14}}>
              <div style={{display:"flex",gap:9,marginBottom:11,flexWrap:"wrap"}}>
                <div style={{flex:1,minWidth:160}}>
                  <label style={{fontSize:9,color:"#6b6b8a",textTransform:"uppercase",letterSpacing:1,display:"block",marginBottom:4}}>Category</label>
                  <select value={category} onChange={e=>setCategory(e.target.value)} style={{width:"100%",background:"#0d0d1a",border:"1px solid #2a2a4a",borderRadius:7,padding:"7px 9px",color:"#e8e8f5",fontSize:12,outline:"none"}}>
                    {CATEGORIES.map(c=><option key={c}>{c}</option>)}
                  </select>
                </div>
                <div style={{flex:1,minWidth:110}}>
                  <label style={{fontSize:9,color:"#6b6b8a",textTransform:"uppercase",letterSpacing:1,display:"block",marginBottom:4}}>Market</label>
                  <select value={market} onChange={e=>setMarket(e.target.value)} style={{width:"100%",background:"#0d0d1a",border:"1px solid #2a2a4a",borderRadius:7,padding:"7px 9px",color:"#e8e8f5",fontSize:12,outline:"none"}}>
              ...
              <div style={{marginBottom:12}}>
                <label style={{fontSize:9,color:"#6b6b8a",textTransform:"uppercase",letterSpacing:1,display:"block",marginBottom:6}}>Sources</label>
                <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                  {SOURCES.map(src=>{const active=activeSources.includes(src.id);return(
                    <button key={src.id} onClick={()=>toggleSource(src.id)} className="src-chip" style={{padding:"4px 10px",borderRadius:20,fontSize:10,fontWeight:600,cursor:"pointer",transition:"all 0.2s",background:active?`${src.color}20`:"#1a1a2e",border:`1px solid ${active?src.color:"#2a2a4a"}`,color:active?src.color:"#6b6b8a"}}>{src.icon} {src.label}</button>
                  );})}
                </div>
              </div>

              <button onClick={runSearch} disabled={loading||activeSources.length===0} className="scan-btn" style={{width:"100%",padding:"11px",borderRadius:9,background:loading?"#2a2a4a":"linear-gradient(135deg,#FF6010,#FF3C00)",border:"none",color:loading?"#6b6b8a":"#fff",fontSize:14,fontWeight:800,cursor:loading?"not-allowed":"pointer",fontFamily:"'Syne',sans-serif",transition:"all 0.2s",animation:!loading&&!currentResults?"glow-pulse 2s infinite":"none",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
                {loading?<><div style={{width:14,height:14,border:"2px solid #6b6b8a",borderTopColor:"#e8e8f5",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/>{statusMsg}</>:<>📡 Run Trend Scan</>}
              </button>
            </div>
            ...
            {currentResults && <div style={{animation:"slide-in 0.4s ease"}}>
              {currentResults.topInsight && (
                <div style={{background:"linear-gradient(135deg,#1a2f1a,#122a12)",border:"1px solid #16803440",borderRadius:10,padding:"10px 14px",marginBottom:12,display:"flex",alignItems:"flex-start",gap:8}}>
                  <span style={{fontSize:16}}>🎯</span>
                  <div>
                    <div style={{fontSize:9,color:"#4ade80",textTransform:"uppercase",letterSpacing:1,marginBottom:2}}>Top Opportunity</div>
              ...
              <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap"}}>
                {[{label:"Products",val:currentResults.products?.length||0,color:"#FF6010"},{label:"Sources",val:activeSources.length,color:"#1877F2"},{label:"Market",val:market,color:"#34A853"},{label:"🔥 Hot",val:(currentResults.products||[]).filter(p=>p.isHot).length,color:"#FBBF24"}].map(st=>(
                  <div key={st.label} style={{flex:1,minWidth:80,background:"#1a1a2e",border:"1px solid #2a2a4a",borderRadius:8,padding:"7px 10px"}}>
              ...
              <div style={{fontSize:9,color:"#6b6b8a",textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>Tap product · Links sorted by sales volume · Expand for trend graph & competitors</div>
              <div style={{display:"flex",flexDirection:"column",gap:7}}>
                {(currentResults.products||[]).map((p,i)=><TrendCard key={i} product={p} index={i} market={market}/>)}
              </div>
              <button onClick={runSearch} style={{marginTop:16,width:"100%",padding:"9px",background:"transparent",border:"1px solid #2a2a4a",borderRadius:8,color:"#6b6b8a",fontSize:12,cursor:"pointer",transition:"all 0.2s"}}
            ...
            {!currentResults&&!loading&&!error&&(
              <div style={{textAlign:"center",padding:"44px 20px"}}>
                ...
                <div style={{fontSize:13,fontWeight:700,color:"#3a3a6a",fontFamily:"'Syne',sans-serif"}}>Ready to scan</div>
                <div style={{fontSize:11,marginTop:4,color:"#3a3a6a"}}>Select category & market, then hit Run Trend Scan</div>
                {history.length > 0 && <button onClick={()=>setView("history")} style={{marginTop:14,padding:"7px 16px",background:"transparent",border:"1px solid #2a2a4a",borderRadius:7,color:"#6b6b8a",fontSize:11,cursor:"pointer"}}>🕐 View Past Scans ({history.length})</button>}
              </div>
            )}
          </>}
          ...
          {view==="history" && <div>
            <div style={{marginBottom:14,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
              <div>
                ...
                <div style={{fontSize:11,color:"#6b6b8a",marginTop:2}}>Last 10 scans — click Load to revisit any scan</div>
              </div>
              {history.length>0&&<button onClick={()=>{setHistory([]);saveHistory([]);}} style={{padding:"5px 11px",borderRadius:7,background:"transparent",border:"1px solid #2a2a4a",color:"#6b6b8a",fontSize:11,cursor:"pointer"}}>Clear All</button>}
            </div>
            {history.length===0?(
              <div style={{textAlign:"center",padding:"44px 20px"}}>
                <div style={{fontSize:36,marginBottom:12,opacity:.4}}>🕐</div>
                <div style={{fontSize:13,fontWeight:700,color:"#3a3a6a",fontFamily:"'Syne',sans-serif"}}>No history yet</div>
                <div style={{fontSize:11,color:"#3a3a6a",marginTop:4}}>Run your first scan to start building history</div>
                <button onClick={()=>setView("results")} style={{marginTop:13,padding:"7px 16px",background:"#FF6010",border:"none",borderRadius:7,color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>Go to Results →</button>
              </div>
            ):(
          ...
          {view==="compare" && <div>
            <div style={{marginBottom:13,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
              <div>
                ...
                <div style={{fontSize:11,color:"#6b6b8a",marginTop:2}}>Run multiple scans to compare side by side</div>
              </div>
              {savedScans.length>0&&<button onClick={()=>setSavedScans([])} style={{padding:"5px 11px",borderRadius:7,background:"transparent",border:"1px solid #2a2a4a",color:"#6b6b8a",fontSize:11,cursor:"pointer"}}>Clear All</button>}
            </div>
            {savedScans.length===0?(
              <div style={{textAlign:"center",padding:"44px 20px"}}>
                <div style={{fontSize:36,marginBottom:12,opacity:.4}}>⚖️</div>
                <div style={{fontSize:13,fontWeight:700,color:"#3a3a6a",fontFamily:"'Syne',sans-serif"}}>No scans to compare</div>
                <div style={{fontSize:11,color:"#3a3a6a",marginTop:4}}>Run 2+ scans — they auto-save here for comparison</div>
                <button onClick={()=>setView("results")} style={{marginTop:13,padding:"7px 16px",background:"#FF6010",border:"none",borderRadius:7,color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>Go to Results →</button>
              </div>
            ):(
          ...
          {view==="export" && <div>
            <div style={{marginBottom:14}}>
              ...
              <div style={{fontSize:11,color:"#6b6b8a",marginTop:2}}>Download or share your trend data</div>
            </div>
            {savedScans.length===0?(
              <div style={{textAlign:"center",padding:"44px 20px"}}>
                <div style={{fontSize:36,marginBottom:12,opacity:.4}}>📂</div>
                <div style={{fontSize:13,fontWeight:700,color:"#3a3a6a",fontFamily:"'Syne',sans-serif"}}>No data yet</div>
                <button onClick={()=>setView("results")} style={{marginTop:13,padding:"7px 16px",background:"#FF6010",border:"none",borderRadius:7,color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>Go to Results →</button>
              </div>
            ):(
              <>
                <div style={{background:"#1a1a2e",border:"1px solid #2a2a4a",borderRadius:10,padding:13,marginBottom:14}}>
                  <div style={{fontSize:9,color:"#6b6b8a",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>Saved Scans ({savedScans.length})</div>
                  {savedScans.map(scan=>(
                    <div key={scan.id} style={{display:"flex",justifyContent:"space-between",padding:"6px 9px",background:"#12122a",borderRadius:6,marginBottom:5}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {[
                    ...
                    <button key={opt.label} onClick={opt.action} className="exp-btn" style={{display:"flex",alignItems:"center",gap:12,padding:"13px 15px",background:"linear-gradient(135deg,#1a1a2e,#12122a)",border:`1px solid ${opt.color}40`,borderRadius:10,cursor:"pointer",transition:"all 0.2s",textAlign:"left"}}
                      ...
                      <div>
                        ...
                        <div style={{fontSize:10,color:"#6b6b8a",marginTop:1}}>{opt.sub}</div>
                      </div>
                      <div style={{marginLeft:"auto",color:"#6b6b8a",fontSize:14}}>↓</div>
                    </button>
                  ))}
                </div>
              </>
